...
function buildWorld() {
  ...
  const set = (x: number, y: number, kind: TileKind) => {
    if (x >= 0 && x < COLS && y >= 0 && y < ROWS) rows[y][x] = { kind };
  };
  const fill = (x1: number, y1: number, x2: number, y2: number, kind: TileKind) => {
    for (let y = y1; y <= y2; y += 1) for (let x = x1; x <= x2; x += 1) set(x, y, kind);
  };
  ...
  for (let x = 0; x < COLS; x += 1) { set(x, 0, "tree"); set(x, ROWS - 1, "tree"); }
  for (let y = 0; y < ROWS; y += 1) { set(0, y, "tree"); set(COLS - 1, y, "tree"); }
  fill(0, 12, 1, 13, "path"); fill(COLS - 2, 12, COLS - 1, 13, "path");
  fill(16, 24, 17, 25, "path"); fill(16, 0, 17, 1, "path");
  fill(2, 2, 4, 4, "bush"); fill(29, 2, 31, 4, "bush"); fill(2, 21, 4, 23, "bush"); fill(29, 21, 31, 23, "bush");
  ...
  fill(11, 5, 22, 6, "path"); fill(13, 6, 20, 7, "path"); fill(12, 17, 21, 18, "path");
  fill(9, 10, 10, 15, "path"); fill(23, 10, 24, 15, "path");
  ...
  fill(10, 6, 23, 7, "path"); fill(10, 15, 23, 16, "path"); fill(10, 7, 11, 15, "path"); fill(22, 7, 23, 15, "path");
  ...
  fill(16, 24, 17, 25, "path"); fill(16, 0, 17, 1, "path"); fill(0, 12, 1, 13, "path"); fill(COLS - 2, 12, COLS - 1, 13, "path");
  return rows;
}
...
function drawGrass(ctx: CanvasRenderingContext2D, x: number, y: number, seed: number) {
  ...
  ctx.fillRect(x + 18, y + 12, 2, 2); ctx.fillRect(x + 28, y + 6, 1, 1);
  ctx.fillRect(x + 34, y + 22, 2, 2);
  ctx.fillStyle = seed % 3 === 0 ? COLORS.grassC : COLORS.grassB;
  ctx.fillRect(x + 9, y + 24, 2, 2); ctx.fillRect(x + 24, y + 10, 2, 2);
}
...
function drawWater(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number) {
  ...
  ctx.fillRect(x + 9, y + 7, 5, 1); ctx.fillRect(x + 23, y + 18, 6, 1);
}
...
function drawFence(ctx: CanvasRenderingContext2D, x: number, y: number) {
  ...
  ctx.fillRect(x + 4, y + 4, 2, 2); ctx.fillRect(x + 16, y + 4, 2, 2);
}
...
function drawPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, dir: Dir, frame: number) {
  ...
  const px = (dx: number, dy: number, color: string) => {
    ...
    ctx.fillStyle = color; ctx.fillRect(x + fx * scale, y + dy * scale, scale, scale);
  };
  const row = (dy: number, start: number, end: number, color: string) => {
    for (let dx = start; dx <= end; dx += 1) px(dx, dy, color);
  };
  const walk = frame === 0 ? 0 : frame === 1 ? 1 : 2;
  const legLeft = walk === 1 ? [4, 5] : walk === 2 ? [3, 4] : [4, 5];
  const legRight = walk === 1 ? [10, 11] : walk === 2 ? [11, 12] : [10, 11];
  if (dir === "down") {
    row(0,6,9,"#1b1b1b"); row(1,5,10,"#1b1b1b"); row(2,6,9,"#c78a4d"); row(3,6,9,"#c78a4d");
    row(4,5,10,"#1b1b1b"); row(4,6,9,"#ef3a3a"); row(5,5,10,"#ef3a3a");
    px(7,6,"#ffffff"); px(8,6,"#ffffff");
  } else if (dir === "up") {
    row(0,6,9,"#1b1b1b"); row(1,5,10,"#1b1b1b"); row(2,6,9,"#1b1b1b"); row(3,6,9,"#1b1b1b");
    row(4,5,10,"#1b1b1b"); row(4,6,9,"#ef3a3a"); row(5,5,10,"#ef3a3a");
    px(7,6,"#ffffff"); px(8,6,"#ffffff");
  } else {
    row(0,6,9,"#1b1b1b"); row(1,5,10,"#1b1b1b"); row(2,5,8,"#c78a4d"); row(3,5,8,"#c78a4d");
    row(4,5,9,"#1b1b1b"); row(4,6,9,"#ef3a3a"); row(5,5,9,"#ef3a3a");
    px(7,6,"#ffffff"); px(8,6,"#ffffff");
  }
  ...
  px(legLeft[0],10,"#1b1b1b"); px(legLeft[1],10,"#2b5bf0");
  px(legRight[0],10,"#1b1b1b"); px(legRight[1],10,"#2b5bf0");
  px(legLeft[0],11,"#1b1b1b"); px(legLeft[1],11,"#2b5bf0");
  px(legRight[0],11,"#1b1b1b"); px(legRight[1],11,"#2b5bf0");
...
function drawFollower(ctx: CanvasRenderingContext2D, x: number, y: number, dir: Dir, frame: number) {
  const scale = SCALE; const flip = dir === "left";
  const px = (dx: number, dy: number, color: string) => {
    const fx = flip ? TILE - 1 - dx : dx; ctx.fillStyle = color; ctx.fillRect(x + fx * scale, y + dy * scale, scale, scale);
  };
  const row = (dy: number, s: number, e: number, color: string) => { for (let dx = s; dx <= e; dx += 1) px(dx, dy, color); };
  ...
  row(2,6,9,dark); row(3,5,10,dark); row(4,4,11,dark); row(5,4,11,dark); row(6,4,11,yellow); row(7,4,11,yellow); row(8,4,11,yellow); row(9,4,11,yellow); row(10,5,10,yellow); row(11,5,10,yellow); row(12,6,9,dark); row(13,7,8,dark);
  px(6,0,dark); px(9,0,dark); row(1,6,9,yellow); px(5,1,dark); px(10,1,dark); px(5,2,yellow); px(10,2,yellow); px(4,3,dark); px(11,3,dark); px(4,4,yellow); px(11,4,yellow);
  px(6,5,dark); px(9,5,dark); px(7,6,dark); px(8,6,dark); px(7,7,yellow); px(8,7,yellow);
  px(4,8,dark); px(11,8,dark); px(5,9,yellow); px(10,9,yellow); px(5,10,yellow); px(10,10,yellow); px(6,11,dark); px(9,11,dark); px(6,12,dark); px(9,12,dark);
  px(2,9,dark); px(1,10,dark); px(0,11,dark); px(1,12,yellow); px(2,12,dark); px(3,11,yellow); px(4,10,yellow); px(5,9,yellow); px(6,8,yellow);
...
function behindPoint(point: Point, dir: Dir) {
  switch (dir) { case "up": return { x: point.x, y: point.y + 1 }; case "down": return { x: point.x, y: point.y - 1 }; case "left": return { x: point.x + 1, y: point.y }; case "right": return { x: point.x - 1, y: point.y }; }
}
...
export default function AmitySquare() {
  ...
  useEffect(() => {
    const onDown = (event: KeyboardEvent) => {
      ...
      if (dir) { event.preventDefault(); keysRef.current.add(key); lastDirRef.current = dir; facingRef.current = dir; }
    };
    const onUp = (event: KeyboardEvent) => {
      const key = event.key.toLowerCase();
      const dir = keyToDir(key);
      if (dir) { event.preventDefault(); keysRef.current.delete(key); }
    };
  ...
  useEffect(() => {
    ...
    const drawWorld = (frame: number, camX: number, camY: number) => {
      ...
      const startCol = Math.max(0, Math.floor(camX / PX));
      const startRow = Math.max(0, Math.floor(camY / PX));
      const endCol = Math.min(COLS - 1, Math.ceil((camX + VIEW_W) / PX));
      const endRow = Math.min(ROWS - 1, Math.ceil((camY + VIEW_H) / PX));
      for (let row = startRow; row <= endRow; row += 1) {
        for (let col = startCol; col <= endCol; col += 1) {
          const tile = world[row][col];
          const sx = col * PX - camX; const sy = row * PX - camY;
    ...
    const step = (time: number) => {
      ...
      if (!movingRef.current) {
        ...
        if (intent) {
          ...
          if (!isSolid(nextX, nextY)) {
            movingRef.current = true; moveFromRef.current = { ...moveToRef.current }; moveToRef.current = { x: nextX, y: nextY };
            moveProgressRef.current = 0; walkFrameRef.current = (walkFrameRef.current + 1) % 3;
          }
        }
      } else {
      ...
      camRef.current.x = Math.max(0, Math.min(camRef.current.x, WORLD_W - VIEW_W));
      camRef.current.y = Math.max(0, Math.min(camRef.current.y, WORLD_H - VIEW_H));
      ...
      hudClock += dt;
      if (hudClock >= DEBUG_RATE) {
        hudClock = 0;
        setHud({ x: Math.round(playerTileX * 10) / 10, y: Math.round(playerTileY * 10) / 10, dir: facingRef.current, moving: movingRef.current, followerX: Math.round(followerRef.current.x * 10) / 10, followerY: Math.round(followerRef.current.y * 10) / 10 });
      }
    ...
    <div className="min-h-screen overflow-hidden text-white" style={{ background: "radial-gradient(circle at top,#1f4b28 0%,#132016 36%,#090b0d 100%)" }}>
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col items-center justify-center gap-4 p-4">
        <div className="flex w-full max-w-[900px] items-end justify-between gap-4">
          ...
          <button type="button" onClick={() => setDebug((value) => !value)} className="rounded-full border border-white/10 bg-black/30 px-4 py-2 text-xs font-medium tracking-wide text-white/85 backdrop-blur transition hover:bg-black/45">
        ...
        {debug ? (
          <div className="grid gap-2 rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-xs text-emerald-100/80 shadow-lg backdrop-blur sm:grid-cols-2">
            <div>player tile: {hud.x.toFixed(1)}, {hud.y.toFixed(1)}</div>
            <div>facing: {hud.dir}</div>
            <div>moving: {hud.moving ? "yes" : "no"}</div>
            <div>route: /amity-square</div>
